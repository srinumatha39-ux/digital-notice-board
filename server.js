...
/* ================================================= */
/* PUSH NOTIFICATIONS (server-side additions)         */
/* Add these near your other requires/setup in         */
/* server.js, alongside your existing Supabase client. */
/* ================================================= */
...
// --- Endpoint: student's browser calls this after subscribing ---
// Assumes you already have `supabase` initialized elsewhere in server.js
app.post("/api/push/subscribe", async (req, res) => {
    const { collegeId, rollNumber, subscription } = req.body;

    if (!collegeId || !subscription || !subscription.endpoint) {
        return res.status(400).json({ error: "Missing collegeId or subscription" });
    }
...
// --- Helper: call this after a notice is successfully created ---
// Example: await sendPushToCollege(collegeId, { title: "New Exam Notice", body: notice.title });
async function sendPushToCollege(collegeId, notice) {
    try {
        const { data: subscriptions, error } = await supabase
            .from("push_subscriptions")
        ...
        const sendPromises = subscriptions.map(async (row) => {
            try {
                ...
                // 410/404 = subscription is dead (student unsubscribed / cleared browser data)
                if (err.statusCode === 410 || err.statusCode === 404) {
                    await supabase
                        .from("push_subscriptions")
                        .delete()
...
// Export if your notice-creation route lives in a different file
module.exports = { sendPushToCollege };

/* ================================================= */
/* Wire it up: in your EXISTING route that creates a  */
/* new notice (after it's successfully saved), add:   */
/*                                                     */
/*   await sendPushToCollege(collegeId, newNotice);    */
/*                                                     */
/* right after the Supabase insert succeeds, using     */
/* the same collegeId the notice was created under.    */
/* ================================================= */
